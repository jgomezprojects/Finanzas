...
btnCambiarNombre.addEventListener("click", () => {
    ...
    if (!nuevo || !nuevo.trim()) {
        mostrarNotificacion("El nombre no puede estar vacío", "warning", 3000);
        return;
    }

    sobreActual.nombre = nuevo.trim();
    guardarDatos();
    renderSobres();
    modal.classList.add("hidden");
...
btnCambiarPorcentaje.addEventListener("click", () => {
    let nuevo = Number(prompt("Nuevo porcentaje:"));
    if (isNaN(nuevo) || nuevo <= 0) {
        mostrarNotificacion("Porcentaje inválido. Debe ser un número mayor a 0", "error", 3000);
        return;
    }
    ...
    if (nuevo > porcentajeDisponible) {
        mostrarNotificacion(`No tienes suficiente porcentaje disponible. Disponible: ${porcentajeDisponible}%`, "error", 4000);
        porcentajeDisponible -= sobreActual.porcentaje;
        return;
    }
...
if (gastarContent) {
    gastarContent.addEventListener("click", (e) => {
            ...
            if (gastoActualConfigurar) {
                ...
                sobres.forEach(sobre => {
                    ...
                    btn.addEventListener("click", () => {
                        ...
                        document.querySelectorAll("#listaSobresConfigurar .fuente-btn")
                            .forEach(b => b.classList.remove("selected"));
                        btn.classList.add("selected");
        ...
        if (btnGastar) {
            ...
            // Buscar el sobre configurado
            const sobreConfigurado = sobres.find(s => s.id === gasto.sobreId);
            if (!sobreConfigurado) {
                mostrarNotificacion("El sobre configurado no existe.", "error", 4000);
                return;
            }
            
            // Verificar que haya suficiente dinero en el sobre
            ...
            // Registrar movimiento
            registrarMovimiento('gasto', `Gasto fijo: ${gasto.nombre}`, gasto.monto, null, sobreConfigurado.nombre, gasto.nombre);
            ...
            mostrarNotificacion(`Gasto de ${formatoCOP(gasto.monto)} realizado desde "${sobreConfigurado.nombre}"`, "success", 3000);
...
function verificarModalesAbiertos() {
    ...
    // Solo ocultar/mostrar navegación
    if (hayModalAbierto) {
