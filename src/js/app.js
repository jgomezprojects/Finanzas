...
function filtrarMovimientosPorFechaYSobre(filtro) {
    ...
    return movimientos.filter(mov => {
        // Solo movimientos que tengan un sobre asociado
        if (!mov.sobre) return false;
...
function calcularEstadisticasPorSobre(movimientosFiltrados) {
    ...
    movimientosFiltrados.forEach(mov => {
        const nombreSobre = mov.sobre;
        
        if (!estadisticas[nombreSobre]) {
            estadisticas[nombreSobre] = {
                nombre: nombreSobre,
                ingreso: 0,
                egreso: 0
            };
        ...
        // Ingresos: tipo 'ingreso' con sobre (excluyendo préstamos)
        if (mov.tipo === 'ingreso' && mov.monto > 0) {
            estadisticas[nombreSobre].ingreso += mov.monto;
        }
        
        // Egresos: tipo 'gasto' con sobre (excluyendo préstamos)
        if (mov.tipo === 'gasto' && mov.monto > 0) {
            estadisticas[nombreSobre].egreso += mov.monto;
        }
    });
...
if (btnResetEstadisticas) {
    btnResetEstadisticas.addEventListener("click", () => {
        requireAuth(() => {
            ...
            // Obtener IDs de los movimientos a eliminar (solo los que tienen sobre y no son préstamos)
            const idsAEliminar = movimientosFiltrados
                .filter(mov => {
                    // Excluir movimientos de préstamos
                    if (mov.descripcion && (
                        mov.descripcion.includes('Préstamo a') || 
                        mov.descripcion.includes('Devolución de préstamo')
                    )) {
                    ...
                    return mov.sobre && (mov.tipo === 'ingreso' || mov.tipo === 'gasto');
                })
                .map(mov => mov.id);
            ...
            if (idsAEliminar.length > 0) {
                ...
                // Recalcular y renderizar estadísticas
                renderEstadisticas();
                
                mostrarNotificacion(`${idsAEliminar.length} movimientos eliminados de estadísticas`, "success", 3000);
            } else {
