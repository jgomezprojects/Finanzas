...
/**
 * Valida la integridad de los datos financieros
...
function validarIntegridadDatos() {
    ...
    const sumaPorcentajes = sobres.reduce((sum, s) => sum + s.porcentaje, 0);
    const porcentajeEsperado = 100 - sumaPorcentajes;
    
    // Corregir porcentajeDisponible si no coincide
    if (Math.abs(porcentajeDisponible - porcentajeEsperado) > 0.01) {
        console.warn(`Corrigiendo porcentajeDisponible. Esperado: ${porcentajeEsperado}%, Actual: ${porcentajeDisponible}%`);
        porcentajeDisponible = Math.max(0, porcentajeEsperado); // No permitir valores negativos
    }
    
    // Validar que porcentajes no excedan 100%
    if (sumaPorcentajes > 100) {
        console.warn(`ADVERTENCIA: La suma de porcentajes excede 100%. Suma: ${sumaPorcentajes}%`);
    }
    
    // Validar que los valores de sobres no sean negativos
    sobres.forEach(sobre => {
        if (sobre.valor < 0) {
            console.warn(`Corrigiendo valor negativo del sobre "${sobre.nombre}". Valor: ${sobre.valor}`);
            sobre.valor = 0;
        }
        if (sobre.porcentaje < 0 || sobre.porcentaje > 100) {
            console.warn(`Porcentaje inválido en sobre "${sobre.nombre}". Porcentaje: ${sobre.porcentaje}%`);
            sobre.porcentaje = Math.max(0, Math.min(100, sobre.porcentaje));
        }
    });
    
    // Validar que las fuentes no tengan valores negativos
    Object.keys(fuentes).forEach(nombreFuente => {
        if (fuentes[nombreFuente] < 0) {
            console.warn(`Corrigiendo valor negativo de la fuente "${nombreFuente}". Valor: ${fuentes[nombreFuente]}`);
            fuentes[nombreFuente] = 0;
        }
    });
    
    // Validar que totalGeneral no sea negativo
    if (totalGeneral < 0) {
        console.warn(`Corrigiendo totalGeneral negativo. Valor: ${totalGeneral}`);
        totalGeneral = 0;
    }
    
    return {
        porcentajesValidos: sumaPorcentajes <= 100,
        porcentajeDisponibleCorrecto: Math.abs(porcentajeDisponible - porcentajeEsperado) <= 0.01,
        ...
        try {
            ...
            if (datos) {
                ...
                if (sobres.length > 0 || totalGeneral > 0 || Object.values(fuentes).some(v => v > 0)) {
                    ...
                    if (typeof mostrarNotificacion === 'function') {
                        mostrarNotificacion('Inicio de sesión exitoso', 'success', 3000);
                    }
                }
            }
...
btnCambiarPorcentaje.addEventListener("click", () => {
    ...
    // Mostrar el mismo porcentaje disponible que en "crear sobre"
    // El sobre puede usar este porcentaje + su porcentaje actual
    porcentajeDisponibleEditar.textContent = `Porcentaje disponible: ${porcentajeDisponible}% (puedes usar hasta ${porcentajeDisponible + sobreActual.porcentaje}% contando el ${sobreActual.porcentaje}% actual de este sobre)`;
...
// ✅ VALIDACIÓN MEJORADA DE PORCENTAJES - Asegura que nunca exceda 100% total
if (btnConfirmarCambiarPorcentaje) {
    btnConfirmarCambiarPorcentaje.addEventListener("click", () => {
        requireAuth(() => {
            ...
            // Este es el porcentaje máximo que se puede asignar a este sobre
            const porcentajeDisponibleParaEseSobre = porcentajeDisponible + sobreActual.porcentaje;

            // Validar que no exceda el porcentaje disponible (incluyendo el del sobre actual)
            ...
            // Calcular suma de todos los porcentajes con el nuevo valor
            const sumaPorcentajesOtros = sobres
                .filter(s => s.id !== sobreActual.id)
                .reduce((sum, s) => sum + s.porcentaje, 0);
            
            const sumaTotal = sumaPorcentajesOtros + nuevo;
            
            if (sumaTotal > 100) {
                const porcentajeMaximoPermitido = 100 - sumaPorcentajesOtros;
                mensajeErrorCambiarPorcentaje.textContent = `El porcentaje excede el máximo permitido. El porcentaje máximo para este sobre es ${porcentajeMaximoPermitido}% para mantener la suma total en 100%`;
                mensajeErrorCambiarPorcentaje.style.display = "block";
                return;
            }

            // Si todo es válido, modificar porcentajeDisponible
            // Liberar el porcentaje anterior del sobre
            porcentajeDisponible += sobreActual.porcentaje;
            // Restar el nuevo porcentaje
            porcentajeDisponible -= nuevo;
            // Actualizar el porcentaje del sobre
            sobreActual.porcentaje = nuevo;

            // Asegurar que porcentajeDisponible no sea negativo (por si hay redondeos)
...
if (btnCancelarEliminarSobre) {
    btnCancelarEliminarSobre.addEventListener("click", () => {
        modalConfirmarEliminarSobre.classList.add("hidden");
        // Mostrar lápices cuando se cancela la eliminación
        document.querySelectorAll(".edit-btn").forEach(btn => {
            btn.style.display = "";
        });
        mostrarBtnAgregarGasto();
...
if (btnConfirmarEliminarSobre) {
    btnConfirmarEliminarSobre.addEventListener("click", () => {
        requireAuth(() => {
            // Obtener datos del sobre antes de eliminarlo
            const montoASobreEliminado = sobreActual.valor;
            const porcentajeDelSobre = sobreActual.porcentaje;
            const nombreEliminado = sobreActual.nombre;
            
            // Redistribuir dinero del sobre eliminado a otros sobres según sus porcentajes
            if (montoASobreEliminado > 0 && sobres.length > 1) {
                // Calcular total de porcentajes de sobres restantes (excluyendo el que se elimina)
                const totalPorcentajeRestante = sobres
                    .filter(s => s.id !== sobreActual.id)
                    .reduce((sum, s) => sum + s.porcentaje, 0);
                
                if (totalPorcentajeRestante > 0) {
                    // Redistribuir proporcionalmente según porcentajes
                    sobres.forEach(s => {
                        if (s.id !== sobreActual.id) {
                            const proporcion = s.porcentaje / totalPorcentajeRestante;
                            s.valor += montoASobreEliminado * proporcion;
                        }
                    });
                } else {
                    // Si no hay porcentaje restante, el dinero se pierde del total
                    totalGeneral -= montoASobreEliminado;
                }
            } else if (sobres.length === 1) {
                // Si es el último sobre, restar el dinero del total
                totalGeneral -= montoASobreEliminado;
            }
            
            // Limpiar referencias en movimientos (actualizar movimientos que referencian este sobre)
            movimientos = movimientos.map(mov => {
                if (mov.sobre === nombreEliminado) {
                    // Marcar como sin sobre específico
                    mov.sobre = null;
                }
                return mov;
            });
            
            // Liberar porcentaje disponible
            porcentajeDisponible += porcentajeDelSobre;
...
// Exponer funciones globalmente para que auth.js pueda usarlas
window.ocultarBtnAgregarGasto = ocultarBtnAgregarGasto;
window.mostrarBtnAgregarGasto = mostrarBtnAgregarGasto;
