...
import { doc, setDoc, getDoc, updateDoc, serverTimestamp, onSnapshot } from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js';
...
export async function saveUserData(uid, userData) {
    try {
        ...
        // Usar serverTimestamp() para obtener la fecha del servidor
        const dataToSave = {
            username: userData.username.trim(),
            email: userData.email.trim(),
            updatedAt: serverTimestamp()  // Timestamp para actualizaciones (siempre se actualiza)
        };

        // Solo agregar createdAt si el documento es nuevo
        if (!documentExists) {
            dataToSave.createdAt = serverTimestamp(); // Timestamp del servidor Firebase (solo al crear)
        }

        // Guardar o actualizar el documento
        // setDoc con merge: true actualiza solo los campos proporcionados
