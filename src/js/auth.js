...
async function procesarAuth() {
    ...
    try {
        ...
        if (modoAuth === 'login') {
            ...
            // En registro, iniciar proceso de verificación por email
            // Guardar datos del registro
            const datosRegistro = { username, email, password };
            
            // Iniciar verificación de email (esto enviará el código y mostrará el modal)
            await iniciarVerificacionEmail(email, datosRegistro);
            
            // Mostrar mensaje informativo
            if (typeof mostrarNotificacion === 'function') {
                mostrarNotificacion('Código de verificación enviado a tu email', 'info', 3000);
            }
            
            return; // Salir temprano, el flujo continúa en el modal de verificación
        }
